...
const TEST_URL = 'http://localhost:5173/#/tools/file-hash'

async function testFileHash() {
  console.log('🚀 启动文件哈希校验工具自动化测试...\n')
  ...
  try {
    ...
    const title = page.locator('h1:has-text("文件哈希校验")')
    ...
    const emptyState = page.locator('text=还没有选择文件')
    ...
    const hashContent = page.locator('.hash-content')
    const scrollbarInfo = await hashContent.evaluate((el) => {
    ...
    // ===== 测试用例 6: 检查霓虹风格（lime 绿色主题）=====
    console.log('🎨 检查霓虹风格（绿色主题）...')
    ...
    await page.screenshot({ 
      path: 'test-file-hash-ui.png', 
      fullPage: true 
    })
    console.log('✅ 截图已保存: test-file-hash-ui.png\n')
    ...
    console.log('请仔细检查:')
    console.log('  - 上传区域是否清晰明显（绿色主题）')
    console.log('  - 图标和文字是否正确显示')
    console.log('  - 霓虹风格是否一致')
    console.log('  - 空状态提示是否友好')
    console.log('  - 与其他工具风格是否协调')
    await page.waitForTimeout(15000)
    
    // ===== 输出测试结果 =====
    console.log('\n' + '='.repeat(70))
    console.log('📊 文件哈希校验工具测试结果')
    ...
    console.log('📊 统计:')
    console.log(`   - 测试用例: 7 个`)
    console.log(`   - 通过: ${7 - issues.length} 个`)
...
testFileHash().catch(console.error)
