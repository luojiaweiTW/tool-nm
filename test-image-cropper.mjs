...
const TEST_URL = 'http://localhost:5173/#/tools/image-cropper'

async function testImageCropper() {
  console.log('🚀 启动图片裁剪缩放工具自动化测试...\n')
  ...
  try {
    ...
    const title = page.locator('h1:has-text("图片裁剪缩放")')
    ...
    const cropperContent = page.locator('.cropper-content')
    const scrollbarInfo = await cropperContent.evaluate((el) => {
    ...
    await page.screenshot({ 
      path: 'test-image-cropper-ui.png', 
      fullPage: true 
    })
    console.log('✅ 截图已保存: test-image-cropper-ui.png\n')
    ...
    console.log('请仔细检查:')
    console.log('  - 上传区域是否清晰明显（粉色主题）')
    console.log('  - 图标和文字是否正确显示')
    console.log('  - 霓虹风格是否一致')
    console.log('  - 空状态提示是否友好')
    console.log('  - 与其他图片工具风格是否协调')
    await page.waitForTimeout(15000)
    
    // ===== 输出测试结果 =====
    console.log('\n' + '='.repeat(70))
    console.log('📊 图片裁剪缩放工具测试结果')
...
testImageCropper().catch(console.error)
